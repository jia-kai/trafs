    ...
    def LLC(cls, spec: ProbSpec):
        ...
        while True:
            ...
            if m * n * k <= 1024**2 * 5:
                break
        lam = geo_interp(1e-4, 1e-1, rng.uniform())
        prob, _ = nsopt.prob.LassoClassification.gen_random(m, n, k, lam, rng=rng)
        return prob
...
def main():
    ...
    parser.add_argument(
        "-p", "--prob-name", type=str, required=True, choices=makers.keys()
    )
    parser.add_argument(
        "-i", "--idx", required=True, type=int, help="index of the instance"
    )
    parser.add_argument(
        "-s",
        "--bench-size",
