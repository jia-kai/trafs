...
class Number:
    ...
    def fmt(cls, num: float, prefer_sci: bool = False) -> str:
        ...
        if abs(num) < 5e-3 or (prefer_sci and abs(num) > 1e3):
            ...
            s = rf"\num{{e{e}}}" if abs(b - 1) < 1e-9 else rf"\num{{{b}e{e}}}"
            return s
        return f"{num:.2f}"
...
class BenchmarkSummarizer:
    ...
    eps_split: typing.ClassVar = [1e-3, 1e-6]

    METHOD_DISP_NAMES: typing.ClassVar = {
        "trafs": r"\tras{}",
        "bundle": "Bundle",
        "gd": "GD",
        "sa2": r"\SAtwo{}",
    ...
    def __init__(self, bench_dir: Path):
        results = {}
        nr_cases = 0
        for prob in ProbMakers.all_makers():
    ...
    def make_bench_table(self) -> pd.DataFrame:
        ...
        col_headers = list(
            itertools.product(
                [rf"$\epsilon={Number.fmt(i)}$" for i in self.eps_split],
                [r"Iter\tnote{a}", r"Time\tnote{a}", r"Solved\tnote{b}"],
            )
        ) + list(
            itertools.product(
                ["Termination"], [r"Iter\tnote{c}", r"Time\tnote{c}", r"Error\tnote{d}"]
            )
        )
        table = []

        for prob in itertools.chain(ProbMakers.all_makers().keys(), ["All"]):
            results: list[dict[str, OptimizationResult]]
            methods: list[str]
            if prob != "All":
                results = self.results[prob]
                methods = [i for i in self.METHOD_DISP_NAMES if i in results[0]]
            else:
                results = list(itertools.chain(*self.results.values()))
                available = set(results[0].keys())
                for r in self.results.values():
                    available &= set(r[0].keys())
                methods = [i for i in self.METHOD_DISP_NAMES if i in available]

            row_headers.extend(
                itertools.product([prob], [self.METHOD_DISP_NAMES[i] for i in methods])
            )
            ...
            for eps_i, eps in enumerate(self.eps_split):
                self._fill_eps_perf(
                    sub_table, prob, eps_i, eps, results, fopts, methods
                )
            ...
            for meth_i, meth_name in enumerate(methods):
                ...
                sub_table[meth_i, col] = f"{np.mean(meth_iter):.0f}"
                sub_table[meth_i, col + 1] = Number(np.mean(meth_time))
                sub_table[meth_i, col + 2] = Number(self._gmean(meth_gap, shift=1e-6))
        ...
        df = pd.DataFrame(
            table,
            index=pd.MultiIndex.from_tuples(row_headers),
            columns=pd.MultiIndex.from_tuples(col_headers),
        )
        return df

    def _compute_speedup(
        self, results: list[dict[str, OptimizationResult]], fopts: npt.NDArray
    ) -> None:
        ...
        for eps in self.eps_split:
            ...
            for ri, r in enumerate(results):
                ...
                def get_time(rm: OptimizationResult, threshold=thresh_i):
                    hist = rm.fval_hist
                    idx = np.argmax(hist <= threshold)
                    assert hist[idx] <= threshold
    ...
    def _fill_eps_perf(
        self,
        sub_table: npt.NDArray[np.object_],
        prob: str,
        eps_i: int,
        eps: float,
        results: list[dict[str, OptimizationResult]],
        ...
        for meth_i in range(len(methods)):
            ...
            sub_table[meth_i, eps_i * 3] = Number(
                self._gmean(meth_iter / min_iter[meth_mask])
            )
            sub_table[meth_i, eps_i * 3 + 1] = Number(
                self._gmean(meth_time / min_time[meth_mask])
            )
        ...
        for i in range(3):
            Number.bold_extreme(
                sub_table[:, eps_i * 3 + i], reduction=max if i == 2 else min
            )
...
class LatexTableProc:
    ...
    def merge_multirow(self, col: int) -> typing.Self:
        ...
        for i, j in zip(table_np, table, strict=True):
            i[:] = j
        table = table_np
        s = 0
        while s < len(table):
            ...
            while t < len(table) and table[t, col] == val:
                t += 1
            if t > s + 1:
                table[s, col] = rf"\multirow[c]{{{t - s}}}{{*}}{{{val}}}"
                table[s + 1 : t, col] = " "
                insert_after[table_lidx[s] - 1] = insert_after[table_lidx[t - 1]] = (
                    rf"\cline{{{col + 1}-{col + 1}}}"
                )
            s = t
        for i, j in zip(table_lidx, table, strict=True):
            lines[i] = " & ".join(j)
    ...
    def add_multirow_header(self, nrow: int, *headers: str) -> typing.Self:
        ...
        s = 2 if lines[1].startswith(r"\toprule") else 1
        for i in range(nrow):
        ...
        for i, j in enumerate(headers):
            ...
            if nrow == 1:
                ...
                cols[i] = rf" \multirow[c]{{{nrow}}}{{*}}{{{j}}} "
        lines[s] = " & ".join(cols)

        return self

    def replace_multicol(
        self, placeholder: str, content: str, ncol: int
    ) -> typing.Self:
    ...
    def remove_header_line(self) -> typing.Self:
        ...
        lines = self.as_lines()
        line = lines[2].strip()
        assert line.startswith("&") and line.endswith(r"\\"), line
        del lines[2]
...
def write_defs(summarizer: BenchmarkSummarizer, df: pd.DataFrame, fout):
    ...
    trafs, gd, bundle = (
        BenchmarkSummarizer.METHOD_DISP_NAMES[i] for i in ["trafs", "gd", "bundle"]
    )
    ...
    wd(
        "trasSolveRateCmp",
        "{:.1f}".format(
            df.loc[("All", trafs), col_rate].val / df.loc[("All", bundle), col_rate].val
        ),
    )
...
def plot_result(df: pd.DataFrame, eps_i, outfile: str):
    meth2id = [
        (v, i) for i, v in enumerate(BenchmarkSummarizer.METHOD_DISP_NAMES.values())
    ]
    ...
    color_palette = ["#E63946", "#457B9D", "#F4A261", "#2A9D8F", "#A8DADC", "#1D3557"]
    ...
    ax.set_xticks(np.arange(len(ProbMakers.all_makers())) + bar_group_width / 2)
    ...
    ax.set_ylabel(
        r"Normalized mean solving time to reach $\epsilon \leq 10^{"
        + str(eps_exp[eps_i])
        + "}$"
    )
    handles = [
        mpatches.Patch(color=color_palette[i], label=label_map.get(label, label))
    ...
    handles.append(
        mpatches.Patch(
            facecolor="white",
            hatch="xxx",
            label="Failed to solve",
            edgecolor="black",
            linewidth=0,
        )
    )
    ax.legend(loc="upper right", fancybox=True, framealpha=0.5, handles=handles)
    ax.set_yscale("log")
    ax.set_ylim(0.2, plot_y.max())
    fig.tight_layout()
    fig.savefig(outfile, metadata={"CreationDate": None}, dpi=300, bbox_inches="tight")


def write_to_latex(summarizer: BenchmarkSummarizer, df: pd.DataFrame, outfile: str):
...
def main():
    ...
    parser.add_argument(
        "--bench-dir", default="bench-out", help="directory to load benchmark results"
    )
    parser.add_argument("-t", "--latex", help="output to latex file")
    parser.add_argument(
        "-p",
        "--plot",
        help="plot the table to files; two files are generated with the given prefix",
    )
    parser.add_argument("--plot-ft", help="file type for plot", default="pdf")
    parser.add_argument(
        "--print", action="store_true", help="print the table to stdout"
    )
